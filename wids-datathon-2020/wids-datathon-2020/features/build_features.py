...
def main(input_filepath, output_filepath):
    ...
    train.loc[train['encounter_id'].isin(samp['encounter_id']), 'apache_4a_hospital_death_prob'] = samp.loc[samp['encounter_id'].isin(train['encounter_id']), 'apache_4a_hospital_death_prob'].values
    
    tmp2 = val[(0 > val['apache_4a_hospital_death_prob']) | (np.isnan(val['apache_4a_hospital_death_prob']))].copy().fillna(-999)
    X2 = tmp2[tmp2.columns.difference(['apache_4a_hospital_death_prob'])]
    X2[X2.columns.difference(categorical_cols+binary_cols)] = X2[X2.columns.difference(categorical_cols+binary_cols)].astype('float32')
    X2[categorical_cols+binary_cols] = X2[categorical_cols+binary_cols].astype('str')
    X2.pop(target_col)
    ...
    samp = pd.concat([samp.reset_index(drop=True), pd.DataFrame(X2['encounter_id'], columns=['encounter_id']).reset_index(drop=True)], axis=1)
    val.loc[val['encounter_id'].isin(samp['encounter_id']), 'apache_4a_hospital_death_prob'] = samp.loc[samp['encounter_id'].isin(val['encounter_id']), 'apache_4a_hospital_death_prob'].values
    
    tmp2 = test[(0 > test['apache_4a_hospital_death_prob']) | (np.isnan(test['apache_4a_hospital_death_prob']))].copy().fillna(-999)
    X2 = tmp2[tmp2.columns.difference(['apache_4a_hospital_death_prob'])]
    X2[X2.columns.difference(categorical_cols+binary_cols)] = X2[X2.columns.difference(categorical_cols+binary_cols)].astype('float32')
    X2[categorical_cols+binary_cols] = X2[categorical_cols+binary_cols].astype('str')
    X2.pop(target_col)
    ...
    samp = pd.concat([samp.reset_index(drop=True), pd.DataFrame(X2['encounter_id'], columns=['encounter_id']).reset_index(drop=True)], axis=1)
    test.loc[test['encounter_id'].isin(samp['encounter_id']), 'apache_4a_hospital_death_prob'] = samp.loc[samp['encounter_id'].isin(test['encounter_id']), 'apache_4a_hospital_death_prob'].values
    
    tmp2 = preds[(0 > preds['apache_4a_hospital_death_prob']) | (np.isnan(preds['apache_4a_hospital_death_prob']))].copy().fillna(-999)
    X2 = tmp2[tmp2.columns.difference(['apache_4a_hospital_death_prob'])]
    X2[X2.columns.difference(categorical_cols+binary_cols)] = X2[X2.columns.difference(categorical_cols+binary_cols)].astype('float32')
    X2[categorical_cols+binary_cols] = X2[categorical_cols+binary_cols].astype('str')
    y2 = tmp2['apache_4a_hospital_death_prob'].astype('float32')
    ...
    samp = pd.concat([samp.reset_index(drop=True), pd.DataFrame(X2['encounter_id'], columns=['encounter_id']).reset_index(drop=True)], axis=1)
    preds.loc[preds['encounter_id'].isin(samp['encounter_id']), 'apache_4a_hospital_death_prob'] = samp.loc[samp['encounter_id'].isin(preds['encounter_id']), 'apache_4a_hospital_death_prob'].values
    ...
    # more often than not when d1_lactate_min is -1000, then it is not dead, even otherwise
    # more often than not when d1_arterial_ph_min is -1000, then it is not dead,  even otherwise
    
    # kmeans = KMeans(n_clusters=2, random_state=0).fit(cl_train)
    # train = pd.concat([train, pd.DataFrame(kmeans.predict(cl_train), columns=['cluster_id'])], axis=1)
    # val = pd.concat([val, pd.DataFrame(kmeans.predict(cl_val), columns=['cluster_id'])], axis=1)
    # test = pd.concat([test, pd.DataFrame(kmeans.predict(cl_test), columns=['cluster_id'])], axis=1)
    # preds = pd.concat([preds, pd.DataFrame(kmeans.predict(cl_preds), columns=['cluster_id'])], axis=1)
    # categorical_cols.append('cluster_id')
    ...
    # logger.info('adding d1_heartrate_min_d1_temp_max_tell')
    # train['d1_heartrate_min_d1_temp_max_tell'] = np.isnan(train['d1_heartrate_min']) & np.isnan(train['d1_temp_max'])
    # val['d1_heartrate_min_d1_temp_max_tell'] = np.isnan(val['d1_heartrate_min']) & np.isnan(val['d1_temp_max'])
    # test['d1_heartrate_min_d1_temp_max_tell'] = np.isnan(test['d1_heartrate_min']) & np.isnan(test['d1_temp_max'])
    # preds['d1_heartrate_min_d1_temp_max_tell'] = np.isnan(preds['d1_heartrate_min']) & np.isnan(preds['d1_temp_max'])
    # categorical_cols.append('d1_heartrate_min_d1_temp_max_tell_1')
    
    top_continuous_cols = list(set([
    ...
    #     ('d1_platelets_min', 'd1_sodium_max'),
    #     ('d1_glucose_min', 'd1_heartrate_max'),
    #     ('d1_platelets_min', 'd1_heartrate_max'),
    #     ('d1_bun_max', 'd1_heartrate_max'),
    #     ('d1_lactate_min', 'd1_heartrate_max'),
    #     ('d1_platelets_min', 'd1_wbc_min'),
    #     ('pre_icu_los_days', 'resprate_apache'),
    #     ('pre_icu_los_days', 'gcs_verbal_apache'),
    #     ('pre_icu_los_days', 'bun_apache'),
    #     ('pre_icu_los_days', 'd1_mbp_noninvasive_min'),
    #     ('pre_icu_los_days', 'd1_sysbp_min'),
    #     ('pre_icu_los_days', 'd1_arterial_ph_min'),       
    #     ('d1_mbp_min', 'd1_mbp_noninvasive_min'),       
    #     ('d1_temp_max', 'd1_heartrate_min'),       
    #     ('age', 'd1_resprate_min'),   
    
    #     ('d1_bun_max', 'd1_glucose_min')
    # ]
    for feature_a, feature_b in list(combinations(top_continuous_cols, 2)):
        #rfr = RandomForestClassifier(random_state=0)
        rfr = KNeighborsClassifier(n_neighbors=5, weights='distance')
        ...
        logger.info(f'classifying {cols}')
        classified_train = pd.DataFrame(rfr.predict_proba(train[cols].fillna(-999))[:,1], columns=[feature_name])
        classified_val = pd.DataFrame(rfr.predict_proba(val[cols].fillna(-999))[:,1], columns=[feature_name])
        classified_test = pd.DataFrame(rfr.predict_proba(test[cols].fillna(-999))[:,1], columns=[feature_name])
        classified_preds = pd.DataFrame(rfr.predict_proba(preds[cols].fillna(-999))[:,1], columns=[feature_name])
        logger.info(f'{classified_train.shape}, {classified_val.shape}, {classified_test.shape}, {classified_preds.shape},')
        
        train = pd.concat([train, classified_train], axis=1)
        val = pd.concat([val, classified_val], axis=1)
        test = pd.concat([test, classified_test], axis=1)
        preds = pd.concat([preds, classified_preds], axis=1)
    ...
    preds['apache_2_bodysystem_is_undefined'] = preds['apache_2_bodysystem'].apply(lambda x: True if x in common_cols else False)
