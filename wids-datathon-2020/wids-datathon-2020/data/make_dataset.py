...
def main(is_final_model, is_semi_supervised_model, input_filepath, output_filepath, external_filepath):
    ...
    SEMI_SUPERVISED_DATA = Path.cwd().joinpath(input_filepath).joinpath('semi-supervised.csv')
    ...
    #df.loc[df['encounter_id'].isin(prob['encounter_id']), 'apache_4a_hospital_death_prob'] = prob.loc[prob['encounter_id'].isin(df['encounter_id']), 'apache_4a_hospital_death_prob'].values
